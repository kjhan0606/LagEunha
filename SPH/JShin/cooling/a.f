...
c    You need to include some of these common variables.
        ...
        print *,myid,'before readcoolingdata'
        call readcoolingdata(myid)
        print *,myid,'before getcoolheatredshift'
        call getcoolheatredshift(redshift)
        print *,myid,'before calcevoldata'
        call calcevoldata(global_dt,n_shd,myid,nproc)
        print *,myid,'before dumping'
        gasstep = (6.-4.)
        gasndenstep = 4.
        if(myid.eq. 0) then
        do j = 1, 128
           gasnden = j*gasndenstep + 0.
           gasnden = 10**gasnden
        do i = 1, 128
           gastemp = i*gasstep + 4.
           gastemp  = 10**gastemp
           call getdeltae(gastemp,gasnden,1.E-4, 1,1,finalde)
		   energy(i,j) = finalde
        enddo
        enddo
		open(19,file='energy.dat',form='binary')
		write(19) ((energy(i,j),i=1,128),j=1,128)
		close(19)
        endif
...
ccc    Read heating/cooing data.
        ...
        dimension temp_c_log(n1,n2,n3,n4)
        dimension temp_h_log(n1,n2,n3,n4)
     ...
     &               n_n=1,num_n),n_z=1,num_z),n_r=1,num_r)
          ...
          do i=1,num_t 
            do j=1,num_n
              do k=1,num_z
                do l=1,num_r
                  temp_c_log(i,j,k,l)=cool_UV_log(i,j,k,l)
                  temp_h_log(i,j,k,l)=heat_UV_log(i,j,k,l)
                enddo 
              enddo 
            enddo
          enddo
     ...
     &                       MPI_COMM_WORLD,ierror)
        ...
        call MPI_Bcast(temp_c_log,n1*n2*n3*n4,
     &          MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ierror)
        call MPI_Bcast(temp_h_log,n1*n2*n3*n4,
     &          MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ierror)
     ...
     &         MPI_COMM_WORLD,ierror)
        ...
        do i=1,num_t    
          do j=1,num_n
            do k=1,num_z
              do l=1,num_r
                cool_UV_log(i,j,k,l)=temp_c_log(i,j,k,l)
                heat_UV_log(i,j,k,l)=temp_h_log(i,j,k,l)
              enddo 
            enddo
          enddo
        enddo
