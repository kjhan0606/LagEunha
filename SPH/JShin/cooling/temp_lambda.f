...
c       Common/cool_5 contains cooling/heating data of the redshift.
       call readcoolingdata(0)
       redshift = 44.0
       gas_temp = 44.5465
       gas_nden = 2.74475e-26/(1.23*1.672e-24)
       gas_metl = 1e-10
       UV_shield = 0
...
c    UV_shield indicates self-shieling of cloud from UV. (1:yes,0:no)
        call getcoolheatfinal(gas_temp,gas_nden,gas_metl,
     +                       UV_shield,cooling_rate,heating_rate)

        coolheat = cooling_rate + heating_rate
c       ----
       write(*,*) 'Cooling [erg s-1 cm-3]=', cooling_rate
       write(*,*) 'Heating [erg s-1 cm-3]=', heating_rate
        stop
        end
c---------------------------------------------------------------------
c       subroutine getcoolheatfinal(temp,dens,metal,UV_shield,
c    &           coolrate,heatrate)
c       implicit real*8 (a-h,o-z)
c       integer UV_shield
c       call get_cool_heat_final(temp,dens,metal,UV_shield,coolrate,
c    &        heatrate)
c       return
c       end
c---------------------------------------------------------------------
