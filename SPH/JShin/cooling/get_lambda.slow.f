...
c        do l=1,nn
     ...
     +                              mpi_comm_world,status,ierr)
             call mpi_recv(rrr,nn,mpi_double_precision,isrc,itag3,
     +                            mpi_comm_world,status,ierr)
c            if(isnan(rrr)) then
...
c            endif
             if(n_shd.eq.0) then
			   do ii = 1, nn
               delta_e0(POS(2),POS(3),ii,POS(1))=rrr(ii)
			   enddo
             else    
			   do ii = 1, nn
               delta_e1(POS(2),POS(3),ii,POS(1))=rrr(ii)
			   enddo
             endif
             num_recv = num_recv + 1
           enddo
...
c            print *,'sending finish signal',ifinish,isrc
     ...
     +                               mpi_comm_world,status,ierr)
	         do ii = 1, nn
             call calcft(POS(1),POS(2),POS(3),ii,global_dt,n_shd,rr(ii))
			 enddo
             call mpi_send(irep,1,mpi_integer,0,itag1,
     +                               mpi_comm_world,ierr)
             call mpi_send(POS,4,mpi_integer,0,itag3,
     +                               mpi_comm_world,ierr)
             call mpi_send(rr,nn,mpi_double_precision,0,itag3,
     +                               mpi_comm_world,ierr)
           endif
         enddo
        endif
