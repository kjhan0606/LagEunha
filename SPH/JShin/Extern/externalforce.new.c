...
#define GetExtAccel(type,X0,Y0,Z0) {\
	...
	double pmassinMsun,meancellmass,fs,fr;\
	double fscale,M_b,a_b,G_b,acc,epsilon2;\
	ellinMpc = simpar.boxsize/simpar.nx;\
	ellinkpc = ellinMpc * 1000.;\
	rscale = 1./ellinkpc;\
	pmassinMsun=234988.0;\
	meancellmass=pmassinMsun/pow(simpar.nx/64.L,3.L);\
	...
	a_b=10.0*3.5;\
	G_b=43013.7/1.E10L;\
	epsilon2 = pow(simpar.EPSILON/rscale,2.L)*0;\
	for(i=0;i<simpar.##type##.np;i++){\
		x = (simpar.##type##.u.tbp+i)->x-X0;\
		y = (simpar.##type##.u.tbp+i)->y-Y0;\
		z = (simpar.##type##.u.tbp+i)->z-Z0;\
		double r = sqrt(x*x+y*y+z*z)/rscale;\
		acc = -G_b*M_b/(pow(a_b+r,2.L)+epsilon2)*fscale/simpar.nx/simpar.nx;\
		acc=acc/r;\
		float ax,ay,az;\
		ax = x * acc;\
		ay = y * acc;\
