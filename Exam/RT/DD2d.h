...
#define EunhaParticleFuncs2D(vorotype) \
	...
	GenerateInsideRange(xpinner, vorotype,x,PosType);\
	GenerateInsideRange(ypinner, vorotype,y,PosType);\
...
#define Dcompare2D(compare,vorotype,direction) \
	int vorotype##direction##compare##2D_rt(GridInfo *gridinfo,const void *a, const void *b){\
		postype _a,_b;\
...
#define CopyDDFuncDDInfoFromTYPE1(simpar,TYPE1,type1, TYPE, type) do{\
    ...
    TYPE##_DDFUNC(simpar).xcompare = type##particletypexcompare2D_rt;\
    TYPE##_DDFUNC(simpar).ycompare = type##particletypeycompare2D_rt;\
