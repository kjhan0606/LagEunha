...
int sminitialize(FILE *wp, float xmin, float xmax, float ymin, float ymax){
	...
	i = 72+256*(117+256*174);
  	fprintf(wp,"ctype = CTYPE() concat %d\n", i);
    fprintf(wp,"ctype = CTYPE(STRING) concat 'DuskBlue'\n");
	for(i=0;i<256;i++){
...
int setpolygoncolor(int nlog, float val){
	int res;
	nlog = exp(nlog);
// This part is specific to the model test.
	val = val-1; // this is for the (1,2) of the RT test
//    val = val/2.; // this is for (0,2) density distribution
	if(nlog>0){
...
//    fprintf(wp,"location 27050 28550 5000 27000\n");
    fprintf(wp,"location %d %d %d %d\n",xlocmax+50, xlocmax+1550, ylocmin,ylocmax);
...
int main(int argc,char **argv){
	...
	if(argc ==2) {
		char infile[190];
		sprintf(infile,"rtout.%.6d.dat", atoi(argv[1]));
		fp = fopen(infile,"r");
		wp = fopen("a.sm","w");
	}
	else if(argc <= 7){
		fp = fopen(argv[1],"r");
        if(argc >=3) wp = fopen(argv[2],"w");
        if(argc >=4 )xmin = atof(argv[3]);
        if(argc >=5)xmax = atof(argv[4]);
...
//	for(i=0;i<np;i++) bp[i].w2 = 0.;
	...
	postype xwidth = (xmax-xmin);
	postype ywidth = (ymax-ymin);
...
//				ibp->den = ibp->mass/meanvol;
				...
				*/
				...
				if(1) fprintf(wp,"ctype default  connect x y\n");
				fprintf(wp,"delete x\n");
				fprintf(wp,"delete y\n");
				fprintf(wp,"ptype 0 0\n");
//				if(bp[p[i].id].iregion==1 || bp[p[i].id].iregion==3) fprintf(wp,"ctype default relocate %g %g dot\n", p[i].x, p[i].y);
				fprintf(wp,"ctype default relocate %g %g dot\n", p[i].x, p[i].y);
//				else if(bp[p[i].id].iregion==2) fprintf(wp,"ctype yellow  relocate %g %g dot\n", p[i].x, p[i].y);
//
				if(PINDX(bp+i) == 58443 )fprintf(wp,"expand 0.1 lweight 1 draw_arrow %g %g %g %g expand 1\n", p[i].x, p[i].y, p[i].x+(p[i].ax*dt), p[i].y+(p[i].ay-0.5)*dt);
				if(0){
					postype asize = sqrt(p[i].volume);
					fprintf(wp,"expand 0.4 lweight 1 relocate %g %g label %ld %7.3g %7.3g\n", 
							p[i].x-0.4*asize, p[i].y-0.25*asize, PINDX(p+i), sqrt(p[i].w2), p[i].pressure);
				}
	...
	if(1){
		fprintf(wp,"ctype DuskBlue  expand 0.1 lweight 0\n");
		for(i=0;i<np;i++){
			if(bp[i].x>=xmin && bp[i].x <xmax && bp[i].y >=ymin && bp[i].y < ymax) {
				fprintf(wp,"circle %g %g %g \n", bp[i].x, bp[i].y, sqrt(bp[i].w2));

//				if(PINDX(bp+i) == 58443)
				if(0){
					postype asize = sqrt(bp[i].volume);
					fprintf(wp,"expand 0.4 lweight 1 relocate %g %g label %ld %7.3g %7.3g\n", 
							bp[i].x-0.4*asize, bp[i].y-0.25*asize, PINDX(bp+i), sqrt(bp[i].w2), bp[i].ie);
				}
			}
		}
		fprintf(wp,"ctype default\n");
	}
	fprintf(wp,"ctype default expand 1 lweight 2 box \n");
	{
