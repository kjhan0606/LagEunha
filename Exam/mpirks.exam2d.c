...
static float cputime0[1024];
static float cputime1[1024];


/* generate all the model functions */
generateAllDDFunc2D(vorork4);
...
void migrateVorork4Particles(SimParameters *simpar){
...
void migrateVoroParticles(SimParameters *simpar){
    MPI_Allreduce(&VORO_NP(simpar), &VORO_TNP(simpar), 1, MPI_PTRDIFF_T, MPI_SUM, MPI_COMM(simpar));
    if(VORO_TNP(simpar)>0){
...
void migrateTreeVoroParticles(SimParameters *simpar){
    MPI_Allreduce(&VORO_NP(simpar), &VORO_TNP(simpar), 1, MPI_PTRDIFF_T,
            MPI_SUM, MPI_COMM(simpar));
    if(VORO_TNP(simpar)>0){
...
void paddingVorork4Particles(SimParameters *simpar, postype width){
	int ndim = 2;
	int i;
    ...
    ppadding(VORORK4_BP(simpar), VORO_NP(simpar), (void**)(&VORORK4_BPP(simpar)), 
			...
			SIMBOX(simpar), width, &GRIDINFO(simpar),
			ndim);
#ifdef _OPENMP
...
void paddingTreeVoroParticles(SimParameters *simpar, postype width){
    int ndim = 2;
    int i;
    ...
    ppadding(VORO_TBP(simpar), VORO_NP(simpar), (void**)(&VORO_TBPP(simpar)), &VORO_NPAD(simpar),
            TVORO_DDINFO(simpar), NDDINFO(simpar), SIMBOX(simpar), width, &GRIDINFO(simpar),
            ndim);
...
void  doDomainDecomp2D(SimParameters *simpar, int iflag){
    ptrdiff_t np, tnp;
    ...
    float percent = (float)np/(float)tnp*NID(simpar)*100.;
    float minper, maxper;
