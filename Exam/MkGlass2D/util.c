...
void makeGlassPressure(treevorork4particletype *bp){
	bp->pressure = pow(bp->mass/bp->volume,Gamma) ;
}
	...
	for(j=0;j<ny;j++){
		...
		for(i=0;i<nx;i++){
			...
			if(x >=xmin && x < xmax && y>=ymin && y<ymax){
				xx = x + 1.0*ran3(&iseed)*Lx/(postype)nx;
				yy = y + 1.0*ran3(&iseed)*Ly/(postype)ny;
				xx = fmod(xx+Lx, Lx);
				yy = fmod(yy+Ly, Ly);
				size_t indx = i+nx*j;
				postype vx,vy;
				...
				res[np].w2old  = gl2d_w2Measure(simpar,meand, res[np].pressure, res[np].den);
...
//
		...
		*/
		...
		res[i].w2old = MIN(res[i].w2, res[i].w2ceil);
	}
	VORO_NPAD(simpar) = 0;
	...
	/*
	DEBUGPRINT("P%d has p%d with bp[%d]: w2, w2ceil = %g %g\n",MYID(simpar), 0, 0,
            res[0].w2, res[0].w2ceil);
    exit(9);
	*/
