...
//        if(bp[i].u4if.Flag[ENDIAN_OFFSET] >0)
        {
            ...
            bp[i].rk4.k2y = (bp[i].vy + bp[i].rk4.k1vy/2)*Dtime;
            bp[i].rk4.k2vx = bp[i].ax*Dtime;
            bp[i].rk4.k2vy = bp[i].ay*Dtime;
            bp[i].rk4.k2ie = bp[i].die*Dtime;

            bp[i].x += bp[i].rk4.k2x*0.5;
            bp[i].y += bp[i].rk4.k2y*0.5;
            bp[i].vx += bp[i].rk4.k2vx*0.5;
            bp[i].vy += bp[i].rk4.k2vy*0.5;
            bp[i].ie += bp[i].rk4.k2ie*0.5;
...
//        if(bp[i].u4if.Flag[ENDIAN_OFFSET] >0)
        {
            ...
            bp[i].rk4.k3y = (bp[i].vy + bp[i].rk4.k2vy/2)*Dtime;
...
//        if(bp[i].u4if.Flag[ENDIAN_OFFSET] >0)
        {
            if(PINDX(bp+i) ==43648 || PINDX(bp+i) == 43648-256 || PINDX(bp+i) == 43904+256)
                DEBUGPRINT("p%d has x/y = %g %g vy = %g : %g %g %g %g\n",
                    i, bp[i].x,bp[i].y,
                    bp[i].vy*Dtime,
                    bp[i].rk4.k1y,
                    bp[i].rk4.k2y,
	...
	for(i=0;i<VORO_NP(simpar);i++){
		bp[i].vx *= 0.5;
		bp[i].vy *= 0.5; // inserted to drag the motion.
...
double gl2d_vph2D(SimParameters *simpar){
	...
	BASICCELL_CELLWIDTH(simpar) = cellsize = GL2D_GridSize(simpar);
	BASICCELL_INVCELLWIDTH(simpar) = 1./cellsize;
	...
	VORO_BASICCELL(simpar)= (CellType*)malloc(sizeof(CellType)*mx*my);
	...
	*/
	...
	DEBUGPRINT("P%d has viscosity parameters %g %g: mx/y = %d %d: cellsize= %g\n", MYID(simpar),alphavis, betavis,
			mx,my, cellsize);
