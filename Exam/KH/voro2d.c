...
static postype p0 = 2.5;
static postype rho0 = 1;
double kh_w2Measure(SimParameters *simpar, double dmean, double pressure,double rho){
	double res = KH_Kappa(simpar)*dmean*pow(pressure/p0*rho0/rho,0.5);
    double res2 = res*res;
    return res2;
}

postype kh_Voro_Pressure2D( postype pi, postype pj, Voro2D_Corner *tmp, 
		Voro2D_point *neighwork, SimParameters *simpar){
	postype res;
	res = ( 0.5-KH_OA(simpar)/3.)*(pi+pj);
	postype pkm, pkp;
	...
	res = res + KH_OA(simpar)/3.* (pkm+pkp);
	return res;
}
...
#pragma omp parallel for 
	...
	/*
	DEBUGPRINT("**P%d has p%ld w2 values P= %g w2= %g w2ceil = %g den= %g vol= %g Cs= %g\n", 
			MYID(simpar), PINDX(bp),bp[0].pressure, bp[0].w2, bp[0].w2ceil, bp[0].den,
			 bp[0].volume, bp[0].csound);
			 */
}


double kh_vph2D_rk4(SimParameters *simpar){
	treevorork4particletype *bp = VORORK4_TBP(simpar);
	...
	updateDenW2Pressure2D(simpar,xmin,ymin,xmax,ymax,
			Gamma, paddingTreeVorork4Particles,searchCellRk4Neighbors2D, findCellRk4BP2D);
    Dtime = getAccVoro2D(simpar, xmin, ymin, xmax, ymax,
        OrderOfAccuracy, Courant, Gamma, paddingTreeVorork4Particles,
    ...
    for(i=0;i<VORO_NP(simpar);i++){
		/*
		if(PINDX(bp+i) ==43648 || PINDX(bp+i) == 43648-256 || PINDX(bp+i) == 43904+256) 
				DEBUGPRINT("p%d has x/y = %g %g ay*Dtime : %g %g %g %g\n", 
					i, bp[i].x,bp[i].y,
					bp[i].rk4.k1vy, 
					bp[i].rk4.k2vy,
					bp[i].rk4.k3vy, 
...
double kh_vph2D(SimParameters *simpar){
	postype boxsize = BOXSIZE(simpar)/NX(simpar)*5;
	treevorork4particletype *bp = VORORK4_TBP(simpar);
	int nbp = VORO_NP(simpar);
	int isave = -1;
	...
	pwidth = BASICCELL_CELLWIDTH(simpar) = cellsize = KH_GridSize(simpar);
