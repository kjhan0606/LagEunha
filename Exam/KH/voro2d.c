...
#pragma omp parallel for 
    ...
    for(iy=0;iy<my;iy++){
        ...
        for(ix=0;ix<mx;ix++){
            int np;
            treevorork4particletype *p = kh_Voro2D_FindCellBP(simpar,ix,iy,&np);
            int nneigh;
            Voro2D_point *neighbors = kh_Voro2D_FindNeighbor(simpar,ix,iy,&nneigh);
...
double kh_vph2D_rk4(SimParameters *simpar){
	...
	updateDenW2Pressure2D(simpar,xmin,ymin,xmax,ymax,
			Gamma, paddingTreeVoroParticles);
