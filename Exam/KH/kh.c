...
void kh_readdata(SimParameters *simpar, postype *t, int nstep){
	char infile[190];
...
//
		iflag = t * 1.;
		jflag = (t-dt) * 1;
		if(iflag != jflag || icount%500 == 0)
		{
			kh_outdata(simpar,  icount, t, dt);
//			kh_makemap(simpar, icount);
			void kh_maketscmap(SimParameters *, int);
			kh_maketscmap(simpar, icount);
		}

		dt = kh_vph2D_rk4(simpar);
		t += dt;
		if(MYID(simpar)==0){
