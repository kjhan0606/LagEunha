...
//			ndist[i+nximg*j] = idist;
    ...
    if(MYID(simpar)==0){
        ...
        colorizeit(5, img, nximg,nyimg,"rt.sao", outfile);
    }
    free(cells);
    free(map); free(img);
...
*/
// for hydrostatic equilibrium, the pressure at y is obtained by integral of g*rho(y) 
/*
	...
	for(j=0;j<ny;j++){
		...
		for(i=0;i<nx;i++){
			...
			if(x>=xmin && x < xmax && y>=ymin && y < ymax){
				...
				res[np].den  = rho;
				postype  getRTpressure(SimParameters *, postype , postype , postype , postype );
