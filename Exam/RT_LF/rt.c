...
int RunRT_LF(SimParameters *simpar, int icont){
	postype t,dt;
	int np;
	...
	if(icont ==0) {
		...
		postype dt;
		int nstep = icont;
		void rt_readdata(SimParameters *, postype *, int );
...
//		MkLinkedList(bp,np);
		...
		t += dt;
		if(MYID(simpar)==0){
			printf("Time is %g with icount = %d with dt= %g\n",t, icount, dt);
			fflush(stdout);
		}
		/*
		...
		*/
		...
		if(iflag != jflag || icount%1000 == 0 )
		{
			void rt_outdata(SimParameters *, int , postype , postype );
