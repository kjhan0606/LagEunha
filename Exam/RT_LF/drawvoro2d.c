...
void MkLinkedListSerial(treevoroparticletype *bp, int np){
    int i,j,k;
	int mx,my;
	mx = Nx/4;
	my = Ny/4;
...
Voro2D_point *Voro2D_FindNeighborSerial(int ix, int iy, int *nneigh, treevoroparticletype *bp){
    int i,j,k;
    int np,mp;
    Voro2D_point *neigh;
...
treevoroparticletype *Voro2D_FindCellBPSerial(int ix, int iy, int *mp, treevoroparticletype *bp){
    int i,j,k;
    int np;
...
//	for(i=0;i<np;i++) bp[i].w2 = 0.;
	...
	for(iy=iys;iy<iyf;iy++)
	{
        ...
        for(ix=ixs;ix<ixf;ix++){
            int np;
            treevoroparticletype *p = Voro2D_FindCellBPSerial(ix,iy,&np,bp);
            int nneigh;
...
//				ibp->den = ibp->mass/meanvol;
				...
				*/
				...
				fprintf(wp,"ctype %s\n", mycolor[icolor]);
				fprintf(wp,"shade 0 x y\n");
				if(0) fprintf(wp,"ctype default  lweight 1 expand 1 connect x y\n");
				fprintf(wp,"delete x\n");
				fprintf(wp,"delete y\n");
				fprintf(wp,"ptype 0 0\n");
//				if(bp[p[i].id].iregion==1 || bp[p[i].id].iregion==3) fprintf(wp,"ctype white relocate %g %g dot\n", p[i].x, p[i].y);
...
//				fprintf(wp,"expand 0.1 lweight 1 draw_arrow %g %g %g %g expand 1\n", p[i].x, p[i].y, p[i].x+p[i].vx*dt, p[i].y+p[i].vy*dt);
				if(0){
					...
					fprintf(wp,"expand 0.5 lweight 1 relocate %g %g label %ld \n", 
							p[i].x-0.4*asize, p[i].y-0.25*asize, PINDX(p+i));
				}
