double exam2d_vph(SimParameters *, double (*)(SimParameters *, double, double, double),
		void (*)(SimParameters *, postype, postype, postype, postype, postype,
			void (*)(SimParameters *, postype)));
double exam2d_vph_rt(SimParameters *, double (*)(SimParameters *, double, double, double));
...
#define startRkSDD2D(simpar, model) do {\
	DEBUGPRINT("P%d: "#model" is now starting buildDDInfo2D(simpar) in Exam2d.h\n", \
			MYID(simpar));\
...
void migrateTreeVorork4Particles(SimParameters *);
void migrateVorork4Particles(SimParameters *);
...
void paddingVorork4Particles(SimParameters *, postype );
void paddingTreeVorork4Particles(SimParameters *, postype );
...
#define copyDDFuncDDInfoFromTYPE1(simpar,TYPE1,type1, TYPE, type) do{\
    ...
    for(i=0;i<NDDINFO(simpar);i++){\
        ...
        TYPE##_DDINFO(simpar)[i].pivot = (void*)my_malloc(TYPE##_DDINFO(simpar)[i].npivot*TYPE##_DDINFO(simpar)[i].n_size);\
        int j;\
