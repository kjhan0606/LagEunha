...
void KH_BuildDDInfo(SimParameters *simpar){
	int nprime;
    ...
    MakeDoDeFunc2D(voroparticletype, VORO_DDFUNC(simpar), xcompare2D,ycompare2D, 
			xpinner, ypinner, muladd2D, DDRule2D, InSideBox2D, 
			determineEdgePtl, shift2pos2D, VORO_DDINFO(simpar));

    MakeDoDeFunc2D(treevoroparticletype, TVORO_DDFUNC(simpar), xcompare2D,ycompare2D, 
			xpinner, ypinner, muladd2D, DDRule2D, InSideBox2D, 
			determineEdgePtl, shift2pos2D, TVORO_DDINFO(simpar));
}

void KH_AllParticleMigrate(SimParameters *simpar){
...
#ifdef DEBUG
//    DEBUGPRINT("Migration: P%d has %ld with total np= %ld \n", MYID(simpar), 
...
#ifdef DEBUG
 //   DEBUGPRINT("Migration: P%d has %ld \n", MYID(simpar), VORO_NP(simpar));
#endif
}



void KH_AllParticlePadding(SimParameters *simpar, float width){
	int ndim = 2;
	size_t i;
...
void KH_TreeAllParticlePadding(SimParameters *simpar, float width){
	int ndim = 2;
	size_t i;
...
void KH_ExtractLocalDomainVolume(SimParameters *simpar){
    DoDeInfo *ddinfo = VORO_DDINFO(simpar);
    int i,nddinfo = NDDINFO(simpar);
    SimBoxRange box = KH_SIMBOX(simpar);
...
void KH_BuildSimpleRMS(SimParameters *simpar, SimBoxRange box,  size_t n_size, DoDeFunc *ddfunc, DoDeInfo *ddinfo, MPI_Comm Comm){
    MPI_Status status;
    int src,dest;
    ...
    */
    ...
    else compare = ddfunc->ycompare;
    {
        ...
        KH_BuildSimpleRMS(simpar, sbox,  n_size, ddfunc, ddinfo+1,newcom);
...
void StartKHRkSDD(SimParameters *simpar){
	int nprime;
	PrimeNumber prime[100];
	nprime = getprimenumber(NID(simpar), prime);

    KH_BuildDDInfo(simpar);
    NDDINFO(simpar) = 0;
	/*
	...
	*/
    KH_SIMBOX(simpar).x.min = KH_XMIN(simpar);
    KH_SIMBOX(simpar).x.max = KH_XMAX(simpar);
    KH_SIMBOX(simpar).y.min = KH_YMIN(simpar);
    KH_SIMBOX(simpar).y.max = KH_YMAX(simpar);
    KH_SIMBOX(simpar).z.min = 0;
    KH_SIMBOX(simpar).z.max = 0;

    KH_BuildSimpleRMS(simpar, KH_SIMBOX(simpar), sizeof(voroparticletype), &VORO_DDFUNC(simpar),
            VORO_DDINFO(simpar), MPI_COMM(simpar));


    KH_ExtractLocalDomainVolume(simpar);
    /*
    ExtractLocalDomainVolume(DM_DDINFO(simpar), NDDINFO(simpar),KH_SIMBOX(simpar));
    */

//    BroadCastDDFuncAndDDInfo(simpar);
	CopyDDFuncDDInfoFromTYPE1(simpar, VORO, voro, TVORO, treevoro);
}
void  DomainDecomp2D(SimParameters *simpar, int iflag){
    ptrdiff_t np, tnp;
