...
int makemap2D(SimParameters *simpar, int icount){
	...
	/*
	DEBUGPRINT("P%d has mx/y= %d %d : Lx/y= %g %g : ymax/min= %g %g cellsize= %g\n", 
			MYID(simpar), mx,my,Lx,Ly, ymax,ymin, cellsize);
			*/
	...
	if(MYID(simpar)==0){
		char outfile[189]; 
		sprintf(outfile,"khmap.old.%.6d.ppm", icount); 
	...
	*mp = nbp;
	...
	for(i=0;i<nbp;i++){
		...
		if(GAS_Kappa(simpar) <0) {
			res[i].w2 = - GAS_Kappa(simpar);
			res[i].w2old = - GAS_Kappa(simpar);
		}
		else {
		    res[i].avgNeighboringPressure = res[i].pressure;
			res[i].w2 = kh_w2Measure(simpar,dmean, res[i].pressure, res[i].den);
			res[i].w2old = kh_w2Measure(simpar,dmean, res[i].pressure, res[i].den);
			res[i].w2ceil = res[i].w2;
//        	res[i].w2 = MIN(res[i].w2, res[i].w2ceil);
		}
	}
...
void MkLinkedList(SimParameters *simpar){
	...
	for(i=0;i<VORO_NP(simpar);i++){
		...
		if(index >=mx*my){
			DEBUGPRINT("error detected: %ld : %d %d : %g %g %g %g with cellsize= %g\n", 
					index, mx,my, bp[i].x, bp[i].y, xmin,ymin, cellsize);
		}
		struct linkedlisttype *tmp = cells[index].link;
		cells[index].link = (struct linkedlisttype*)(bp+i);
		cells[index].nmem ++;
