/*
icc -o sph.gizmo.exe sph.gizmo.c -lm
*/
#include<stdio.h>
#include<stdlib.h>
#include<stddef.h>
...
void gethsml(gasparticle *bp, int np, int inow, float *rdist){
	...
	for(i=1;i<=nneigh;i++){
		...
		if(ii<0) {
			ii = -ii-1;
			rdist[ndist++] = bp[inow].x -xmin + ( bp[ii].x-xmin );
		}
		else if(ii<np) {
			...
			ii = np - (ii-np) -1;
			rdist[ndist++] = fabs(xmax-bp[inow].x+xmax-bp[ii].x);
		}
		if(ij<0) {
			ij = -ij-1;
			rdist[ndist++] = (bp[inow].x + bp[ij].x - 2*xmin);
		}
		else if(ij<np) {
			...
			ij = np - (ij-np) -1;
			rdist[ndist++] = fabs(xmax-bp[inow].x+xmax-bp[ij].x);
		}
	}
...
float getax(gasparticle *bp, int np, int inow){
	...
	for(i=1;i<nneigh;i++){
		for(ii=inow-i;ii<=inow+i; ii+=2*i){
			if(ii<0) {
				int ij = -ii-1;
				float dist = bp[inow].x + bp[ij].x -2*xmin;
...
//					double muab = bp[inow].hsml * (bp[inow].vx-bp[ii].vx)*(bp[inow].x-bp[ii].x)/(dist*dist+eta*eta); 
				...
				int	ij = np - (ii-np) -1;
				float dist = fabs(xmax - bp[inow].x + xmax -bp[ij].x);
                float hav = 0.5*(bp[inow].hsml+bp[ij].hsml);
				int ifactor = -1;
                if(dist < hav*2) {
                    double cab,denab,PIab;
...
//					double muab = bp[inow].hsml * (bp[inow].vx-bp[ii].vx)*(bp[inow].x-bp[ii].x)/(dist*dist+eta*eta); 
				...
				int ij = np - (ii-np)-1;
                float dist = (xmax- bp[ij].x) + xmax -bp[inow].x;
                float hav = 0.5*(bp[inow].hsml+bp[ij].hsml);
                float dvel = bp[inow].vx + bp[ij].vx;
				int ifactor = -1;
                if(dist < hav*2) { 
...
void  getden(gasparticle *bp, int np, int inow){
	...
	for(i=1;i<nneigh;i++){
		for(ii=inow-i;ii<=inow+i;ii+=2*i){
			if(ii<0) {
				int ij = -ii-1;
				float dist = fabs(bp[inow].x + bp[ij].x - 2*xmin);
...
double sph(gasparticle *bp, int np){
	...
	for(i=0;i<np;i++){
		getden(bp,np,i);
		bp[i].csound = sqrt(pgamma*bp[i].pressure/bp[i].den);
...
int main(int argc, char **argv){
	...
	double xsep;
	double Lmass, Rmass,  LVol, RVol;
	int i;
	if(0){ /* standard settting */
		Lden = 1;
		Rden = 0.125;
		LP = 1;
		RP = 0.1;
		...
		for(i=0;i<np;i++){
			...
			bp[i].ie = bp[i].pressure/ ( (pgamma-1)*bp[i].den );
			bp[i].vx = 0;
			bp[i].id = i;
		}
	}
