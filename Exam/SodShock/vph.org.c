...
double vph(gasparticle *bp, int np){
	...
	for(i=1;i<np-1;i++){
		...
		double aden2 = 0.5*(bp[i].den + bp[i+1].den);
		double aden1 = 0.5*(bp[i-1].den + bp[i].den);
		double Cs2 = 0.5*(bp[i].Cs + bp[i+1].Cs);
		double Cs1 = 0.5*(bp[i-1].Cs + bp[i].Cs);

		double v1, v2;
		v1 = bp[i-1].vx - bp[i].vx;
		v2 = bp[i+1].vx - bp[i].vx;
		...
		if(0){
			...
			double vis1, vis2;
			vis1 = vis2 = 0;
			if(v2 <0) vis2 = (alpha*Cs2*v2 + beta*v2*v2)/sqrt(1.+eta*eta/d2*d2)*aden2;
			if(v1 >0) vis1 = (alpha*Cs1*v1 + beta*v1*v1)/sqrt(1.+eta*eta/d1*d1)*aden1;
			double viscosity = (vis1+vis2)/2.L/bp[i].mass*area;
			bp[i].ax += viscosity;
