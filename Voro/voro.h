...
#define Voro_Velocity2D(ai,aj,tmp,neighwork,simpar,MODEL) \
	({Voro2D_point res; \
	 res.x=(0.5-MODEL##_OA(simpar)/3.)*(ai->vx + aj->vx) + MODEL##_OA(simpar)/3* \
	  (((treevoroparticletype*) (neighwork[(tmp->upperlink)->upperrelated].vx))+\
	   ((treevoroparticletype*) (neighwork[(tmp           )->lowerrelated].vx))) \
	 res.y=(0.5-MODEL##_OA(simpar)/3.)*(ai->vy + aj->vy) + MODEL##_OA(simpar)/3* \
	  (((treevoroparticletype*) (neighwork[(tmp->upperlink)->upperrelated].vy))+\
	   ((treevoroparticletype*) (neighwork[(tmp           )->lowerrelated].vy)));\
	   res;}\
	 )

#define getwfrac(sqw1,sqw2,sqd12) (0.5L +0.5L*((sqw1-sqw2)/sqd12))
