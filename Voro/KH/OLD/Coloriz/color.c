...
int f2pgm(Img *);
int pgm2col(Img *); 
int pickcol(unsigned char, unsigned char *, unsigned char *, unsigned char*);
int readpgm(unsigned char *, int , int , unsigned char *, unsigned char *, unsigned char *);
int readsao(int , FILE *);
...
int colorizeit(float *map, int nx, int ny, char *outfile){
...
//	ImgHeader(&nx,&ny, outfile);
	...
	{
		initializeImg(&gas, nx, ny, "kh.sao");
		gas.ccdmax0 = 2;
		gas.ccdmin0 = 1.;
		gas.nlog = -1;
		gas.ccd = map;
...
int readpgm(unsigned char *ccd, int nx, int ny, unsigned char *rr, unsigned char *gg, unsigned char *bb) {
	int i, size;
	char line[80];
...
int readsao(int reverse, FILE *saofile) {
	int i;
	int ir, ig, ib, len;
	int n0;
	unsigned char r, g, b;
