...
Voro3D_point voro3D_norm(Voro3D_point *rin){
...
int Voro2D_Trim_Corner(
	...
	for(i=0;i<mp;i++){
		if(voroCorner[i].status == Active){
			...
			/*
			if(voroCorner[i].lowerlink - voroCorner >= mp)
				printf("-Error: mp= %d point= %ld\n",mp,voroCorner[i].lowerlink - voroCorner);
			if(voroCorner[i].upperlink - voroCorner >= mp)
				printf("+Error: mp= %d point= %ld\n",mp,voroCorner[i].upperlink - voroCorner);
				*/
			newmp ++;
		}
	}
...
//		postype t = 0.5L; //This is for Voronoi.
		postype t = wfrac; // This is for Laguerre.
...
int Voro2D_FindVC(
		Voro2D_point *ctrCenter, 
		Voro2D_point *inputneighbors, 
	...
	for(i=0;i<np;i++){
		if(inputneighbors[i].indx != ctrCenter->indx){ 
			...
			neighbors[j].drad2 = 0.25*dist2*(1 + (w2p-neighbors[j].w2)/dist2)
				*(1+(w2p-neighbors[j].w2)/dist2);
			j++;
		}
	}
	np = j;
...
int Voro3D_FindVC(
	...
	for(i=0;i<np;i++){
		if(inputneighbors[i].indx != ctrCenter->indx){ 
			...
			mindist2 = MIN(mindist2, neighbors[j].dist2);
			neighbors[j].w2 = inputneighbors[i].w2;
			postype w2q = neighbors[j].w2;
			postype a = 0.5*(1+(w2p-w2q)/neighbors[j].dist2);
			neighbors[j].drad2 = neighbors[j].dist2*a*a;
			j++;
		}
	}
	np = j;
...
//        Voro3D_Vertex *nnext = getNextPolygonVertex(now,next); 
		...
		postype Pj1 = 0.25*(Pi+neigh[now->related[0]].pressure + neigh[now->related[1]].pressure + neigh[now->related[2]].pressure);
		postype Pj2 = 0.25*(Pi+neigh[next->related[0]].pressure + neigh[next->related[1]].pressure + neigh[next->related[2]].pressure);
		postype Pj12 = 0.3333333*(Pij + Pj1 + Pj2);
		totSij += dSij;
		totPij += Pj12*dSij;
