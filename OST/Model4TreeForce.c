...
float GetPointGrav(float dist){
	int ntmp = dist*invran2nran;
	float fplmf = forcecorrectdiff(ntmp,0) + forcecorrectslope(ntmp,0) * (dist-ntmp*ran2nran);
	return fplmf;
}
void GetCellGrav(float dist, float *fplmf1, float *fplmf2, float *fplmf3){
	int ntmp = dist*invran2nran;
	float tmpdist = dist-ntmp*ran2nran;
