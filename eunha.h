...
#define LAG_OA(simpar) VoroAccuracyOrder(simpar)

typedef struct RTI{
	float rho1,rho2,  deltay, vperturb, Phalf;
	float xmin,ymin,xmax,ymax;
	float OrderofAccuracy;
	int Nxp, Nyp;
}RTI;
...
typedef struct GasInfo{
	...
	float AccuracyOrder, Kappa, fcentroid;
	float dmean, invw2Scale, w2Power, w2Factor;
	int timestepping;
	double g1,g2, rhoc0,meanrho; 
	struct gas_flag flag;
	PosType axExt,ayExt,azExt; // external gravity
	int av_mode;        // 0=Monaghan, 1=NS-stress, 2=two-tier (HLLC+CD10)
...
#define GAS_dtold(simpar) ((simpar)->physics.gasinfo.dtold)
#define GAS_dt(simpar) ((simpar)->physics.gasinfo.dt)
...
#define VoroQ float \
	den,\
	pressure,avgNeighboringPressure,\
	ie,\
	die,\
	/*\
	te,\
	...
	*/\
	...
	dt, \
	volume ,\
	w2 , w2hydro,w2old ,w2ceil // Laguerre circular radius
...
typedef struct vorostressparticletype{
    ...
    VoroQ;
	Stress stress;
    struct treevorostressparticletype *bp;
}vorostressparticletype;

typedef struct treevorostressparticletype{
    struct linkedlisttype *next;
    indxflag u4if;
    char indt[4];
    float x,y,z;
...
typedef struct vorostressrk4particletype{
    ...
    RK4 rk4;
	Stress stress;
    struct treevorostressrk4particletype *bp;
}vorostressrk4particletype;

typedef struct treevorostressrk4particletype{
    struct linkedlisttype *next;
    indxflag u4if;
    char indt[4];
    PosType x,y,z;
...
#define GAS_EVOLMETHOD(simpar) ((simpar)->physics.gasinfo.timestepping)
#define GAS_FCENTROID(simpar) ((simpar)->physics.gasinfo.fcentroid)
...
// NS stress + HLLC + CD10 two-tier blending parameters
#define GAS_AVMODE(simpar) ((simpar)->physics.gasinfo.av_mode)
#define GAS_USEMUSCL(simpar) ((simpar)->physics.gasinfo.use_muscl)
#define GAS_CDAMAX(simpar) ((simpar)->physics.gasinfo.cd_amax)
...
#define KH_Kappa(simpar) ((simpar)->physics.gasinfo.Kappa)
/* RT instability */
...
#define RT_ACCX(simpar) ((simpar)->physics.gasinfo.axExt)
#define RT_ACCY(simpar) ((simpar)->physics.gasinfo.ayExt)
#define RT_ACCZ(simpar) ((simpar)->physics.gasinfo.azExt)
...
#define RT_Kappa(simpar) ((simpar)->physics.gasinfo.Kappa)
/* Kepler test */
...
#define KP_Kappa(simpar) ((simpar)->physics.gasinfo.Kappa)
/* 2D Glass Mading */
...
#define GL2D_ACCX(simpar) ((simpar)->physics.gasinfo.axExt)
#define GL2D_ACCY(simpar) ((simpar)->physics.gasinfo.axExt)
#define GL2D_ACCZ(simpar) ((simpar)->physics.gasinfo.axExt)
...
#define GAS_w2Factor(simpar) ((simpar)->physics.gasinfo.w2Factor)
#define GAS_dMean(simpar) ((simpar)->physics.gasinfo.dmean)
#define GAS_invw2Scale(simpar) ((simpar)->physics.gasinfo.invw2Scale)
#define GAS_ACCX(simpar) ((simpar)->physics.gasinfo.axExt)
#define GAS_ACCY(simpar) ((simpar)->physics.gasinfo.ayExt)
#define GAS_ACCZ(simpar) ((simpar)->physics.gasinfo.azExt)
...
#define NDIM(simpar) ((simpar)->simmodel.ndim)
...
#define VOROflag (1<<6)
#define Wallflag  (1<<7)
#define MAX_INDEX 72057594037927935L
