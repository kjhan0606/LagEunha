...
void FoF_Make_Tree(FoFTStruct *TREE_START, int nnode, FoFTPtlStruct *ptl, int np, int recursiveflag){
	int i;
	...
	else if(recursiveflag == ForLoop){
		FoFTStruct *work;
		for(work=TREE_START;SpareNode-work >0; work++){
			SpareNode = FoF_divide_node(work, SpareNode, ForLoop);
		}
	}
...
#pragma omp parallel 
#endif
		{
			...
			int mys, myf;
			int worksize = (twork + npid - 1)/npid;
			int j;
			mys = worksize *pid;
...
/*
inline enum boolean FoF_InsideOpen(Position p, FoFTStruct *tree, float maxdist) {
...
enum boolean pfof_open(Position p, FoFTStruct *tree, PosType fof_link){
...
size_t pnew_fof_link(Position *p, FoFTStruct *tree, PosType fof_link, Position *linked, size_t nhalo){
	size_t now, ncount = 0;
	void *ptr, *optr, *nptr;
	PosType tmpx,tmpy,tmpz;
			...
			switch(((TYPE*)ptr)->type) {
				case TYPE_TREE:
					if(((FoFTStruct *)ptr)->sibling == ((FoFTStruct *)ptr)->daughter){
						EraeFromTree(optr,ptr,((FoFTStruct *)ptr)->sibling);
