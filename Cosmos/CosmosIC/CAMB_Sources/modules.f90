...
!            CP%Max_eta_k = min(50000*AccuracyBoost,CP%Max_eta_k)
           ...
           if (present(error)) then
              ...
              write(*,'("Om_b h^2             = ",f12.5)') CP%omegab*(CP%H0/100)**2
              write(*,'("Om_c h^2             = ",f12.5)') CP%omegac*(CP%H0/100)**2
              write(*,'("Om_nu h^2            = ",f12.5)') CP%omegan*(CP%H0/100)**2
              write(*,'("Om_Lambda            = ",f12.5)') CP%omegav
              write(*,'("Om_K                 = ",f12.5)') CP%omegak
              if (CP%Num_Nu_Massive > 0) then
                do nu_i=1, CP%Nu_mass_eigenstates 
                 write(*,'(f5.2, " nu, m_nu*c^2/k_B/T_nu0   = ",f10.2," (m_nu = ",f6.3," eV)")') &
...
! Calculation of the visibility function
        ...
        if (CP%AccurateReionization) then                         
          if (FeedbackLevel > 0) write(*,'("Reion opt depth      = ",f11.4)') actual_opt_depth
       end if
