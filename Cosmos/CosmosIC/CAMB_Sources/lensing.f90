...
implicit none
 ...
 logical, parameter :: lens21cm = .false.
...
!$ external OMP_GET_THREAD_NUM, OMP_GET_MAX_THREADS
    ...
    do l=lmin,CP%Max_l
     ! (2*l+1)l(l+1)/4pi C_phi_phi: Cl_scalar(l,1,C_Phi) is l^4 C_phi_phi
       ...
       if (lens21cm)  then
          Cl_scalar(l,in,C_Temp) =  Cl_scalar(l,in,C_PhiTemp+1)
       end if
  ...
  !$OMP SHARED(dtheta,CP,lmax_lensed,roots, npoints,interp_fac,jmax,ls,xl) 
      ...
      do l=2,CP%Max_l
        ...
        Pl= ((2*l-1)* x *pmmp1 - (l-1)*Pmm)/ l
        dP(l) = l*(pmmp1-x*Pl)/sinth**2
        Pmm=pmmp1
        pmmp1=Pl
...
!Approx good to 1e-4 level
         ...
         if (nonpert) then
          cg2fac = llp1*Cg2/2
          fac = (X000**2*BessI(0,cg2fac) - 1)*P(l) + 2*X000**2*( BessI(1,cg2fac)*dm11 + BessI(2,cg2fac)*d2m2)
!           fac = (X000**2*BessI(0,llp1*Cg2/2) - 1)*P(l) + &
!                  2*X000**2*( BessI(1,cg2fac)*dm11 + BessI(2,cg2fac)*d2m2 + BessI(3,cg2fac)*d3m3 )
...
!Otherwise use second order perturbative harmonic method
  ...
  do j=lmin,maxl
     ...
     if (lens21cm)  then
          bare_cls(:,j,CT_Temp)=  Cl_scalar(j,:,C_PhiTemp+1)*norm
       end if
