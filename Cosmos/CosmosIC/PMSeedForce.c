...
void MakeTunnel2Fortran(SimParameters *simpar, enum mtype itype){
	...
	void pkinfowrap4fortran_(float *, float *,  float *, float *, float *, float *, float *, float *, float *, int *, enum mtype *);
...
void PMSeedForce(SimParameters *simpar, DenType *den, DenType *fx, DenType *fy, DenType *fz,
		DenType *den2, DenType *denpad, enum mtype itype){
	...
	*/
...
#else
	MakeTunnel2Fortran(simpar, totm);
	sprintf(matterpower.selectmatter,"total");
#ifdef DEBUG
	if(MYID(simpar)==0) DEBUGPRINT("P%d: now I am here: matter: %s :: %d : %s , npow=%f\n",
			MYID(simpar), matterpower.selectmatter, POWREADFLAG(simpar), powfilename, npow);
#endif
	...
	if(itype ==totm) sprintf(matterpower.selectmatter,"total");
	if(itype ==cdm) sprintf(matterpower.selectmatter,"cdm");
	if(itype ==sph) sprintf(matterpower.selectmatter,"baryon");
			...
			for(j=0;j<ny;j++){
				...
				for(i=0;i<=nx/2;i++){
					...
					if(xksq ==0 || xksq > rknyq2) {
						...
						rk = twopi*sqrt(xksq);
						float velpot = pamp*bpower_(&rk)/rk/rk;
						/*
