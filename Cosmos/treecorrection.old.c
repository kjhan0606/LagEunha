...
size_t Dump2Pos(SimParameters *simpar, 
	...
	*pos = (Position*) malloc(sizeof(Position)*npos);
	Position *p = *pos;
	npos = 0;
	linkedlisttype *tmp = BasicCell[ix+mx*(iy+my*iz)].link;
	while(tmp) {
		if(!IS_FLAG(tmp,BoundaryGhostflag)){
#ifndef GOTPM
...
#endif
			{
				...
				if( IS_FLAG(tmp, DMflag) ) p[npos].mass = TDM_MASS(simpar,0);
				else p[npos].mass = LINKEDTYPE_MASS(simpar,tmp);
#ifndef GOTPM
				if( IS_FLAG(tmp, DMflag)) p[npos].axyz = &(( (treedmparticletype*)tmp)->ax);
				else if(IS_FLAG(tmp, SPHflag)) p[npos].axyz = &(( (treesphparticletype*)tmp)->ax);
				else if(IS_FLAG(tmp, STARflag)) p[npos].axyz = &(( (treestarparticletype*)tmp)->ax);
				else if(IS_FLAG(tmp, AGNflag)) p[npos].axyz = &(( (treeagnparticletype*)tmp)->ax);
#else
...
size_t  Dump2GrvP(SimParameters *simpar, ptrdiff_t ix, ptrdiff_t iy,ptrdiff_t iz,
	...
	*grvp = (TPtlStruct *)malloc(sizeof(TPtlStruct)*ngrv);
	TPtlStruct *g = *grvp;
	ngrv = 0;
	for(k=kmin;k<kmax;k++)for(j=jmin;j<jmax;j++) for(i=imin;i<imax;i++){
		...
		while(tmp){
			...
			if( IS_FLAG(tmp, DMflag)) g[ngrv].mass = TDM_MASS(simpar,0);
...
void free_padding_particles(SimParameters *simpar){
	...
	DM_NPP(simpar) = SPH_NPP(simpar) = STAR_NPP(simpar) = AGN_NPP(simpar) = 0;
}
/*
	...
	*/
...
#ifdef DEBUG
	{
		...
		for(i=0;i<DM_NP(simpar);i++){
			CLEAR_FLAG(bp);
			SET_FLAG(bp, DMflag);
			if(!IS_FLAG(bp,DMflag)){
				DEBUGPRINT("P%d found error in the particle definition: %d\n",MYID(simpar), (bp)->u4if.Flag[_ENDIAN_OFFSET_]);
				exit(999);
			}
			bp++;
		}
	...
	for(ii=0;ii<mx*my*mz;ii++){
		...
		if(npos>0){
...
#ifndef USE_GPU
			{
				...
				float work[linesize*15];
				if(ngrv >=DIRECTSUM) {
					TStruct *TREECELL = (TStruct*)malloc(sizeof(TStruct)*ngrv);
					Make_Tree3(TREECELL,ngrv, grvp, ngrv, theta2,RECURSIVE);
					for(i=0;i<npos;i++){
						treeforce3(simpar, pos+i, TREECELL, linesize, rspheresq, work);
					}
					free(TREECELL);
				}
				else {
