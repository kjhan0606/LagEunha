...
void mpirms(void **ibase,long *mmem, long n_size, DoDeFunc *ddfunc, DoDeInfo *ddinfo,
	...
	{ 
		...
		ddinfo->xchip = ddfunc->xchip;
		ddinfo->ychip = ddfunc->ychip;
		ddinfo->zchip = ddfunc->zchip;
...
void pmigrate(void **ibase,long *mmem, long n_size, DoDeInfo *ddinfo){
	MPI_Status status;
	MPI_Comm com;
	int src,dest;
	long nrecv,nsend;
	...
	pmigrate(&rbase, &nowmem, n_size, ddinfo+1);
...
void ExtractLocalDomainVolume(DoDeInfo *ddinfo, int nddinfo){
	int i;
	ddinfo->u.xyz.xmin = ddinfo->u.xyz.ymin = ddinfo->u.xyz.zmin = -INFINITY;
	ddinfo->u.xyz.xmax = ddinfo->u.xyz.ymax = ddinfo->u.xyz.zmax =  INFINITY;
	for(i=0;i<nddinfo;i++){
		if(i!=0) ddinfo[i].u.r = ddinfo[i-1].u.r;
		if(ddinfo[i].subgroupid==0) {
			ddinfo[i].u.r.rmax[ddinfo[i].idirection] = *(float*)((char*)(&ddinfo[i].pivot[ddinfo[i].subgroupid])+ddinfo[i].xyzchip);
		}
		else if(ddinfo[i].subgroupid != ddinfo[i].nsubgroup-1){
			ddinfo[i].u.r.rmax[ddinfo[i].idirection] = *(float*)((char*)(&ddinfo[i].pivot[ddinfo[i].subgroupid])+ddinfo[i].xyzchip);
			ddinfo[i].u.r.rmin[ddinfo[i].idirection] = *(float*)((char*)(&ddinfo[i].pivot[ddinfo[i].subgroupid-1])+ddinfo[i].xyzchip);
		}
		else {
			ddinfo[i].u.r.rmin[ddinfo[i].idirection] = *(float*)((char*)(&ddinfo[i].pivot[ddinfo[i].subgroupid-1])+ddinfo[i].xyzchip);
		}
