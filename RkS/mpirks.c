...
int getdirection(SimBoxRange box){
	PosType xwidth = box.x.max-box.x.min;
	PosType ywidth = box.y.max-box.y.min;
	PosType zwidth = box.z.max-box.z.min;
	if(zwidth >= xwidth){
		if(zwidth >=ywidth) return 2;
		else return 1;
	}
	else if(xwidth > ywidth) return 0;
	else return 1;
}
...
int SwingSearchID(int a, int b){
	a += (b%2 ==1 ?(1):(-1)) * b;
	return a;
}

void MPI_Guess_kth_value(void *a, ptrdiff_t nmem, ptrdiff_t n_size,  
		int nid, MPI_Comm com, GridInfo *gridinfo, 
	...
	do{
		...
		for(k=0;k<nid;k++){
			...
			if(lowerupper[0].nleft >0 && lowerupper[1].nleft < LONG_MAX) break;
		}
...
void Find_kth_smallest(void *base, ptrdiff_t nmem, ptrdiff_t n_size, 
		DoDeFunc *ddfunc, int kth, int npivot, void *result, int myid, int nid,
	...
	MPI_Guess_kth_value(base,nmem,n_size, nid, com, gridinfo, compare, ntarget, lowerupper, kth, npivot);
...
void mpirks(void **ibase, ptrdiff_t *mmem, ptrdiff_t n_size, DoDeFunc *ddfunc, DoDeInfo *ddinfo,
	...
	{ 
		...
		ddinfo->shift2pos = ddfunc->shift2pos;
	...
	for(igroup=1;igroup<nsubgroup;igroup++){
		...
		if(rbase == NULL) rbase = (void *)my_malloc(sizeof(char)*nrecv*n_size);
...

	{
		...
		mpirks(&base, &nowmem, n_size, ddfunc, ddinfo+1,newcom, gridinfo, dddirection, newrmsbuild);
	...
	*ibase =  base;
...

	{
		...
		mpirks(&rbase, &nowmem, n_size, ddfunc, ddinfo+1,newcom, gridinfo, dddirection, newrmsbuild);
		if(subgroupsize ==1) MPI_Comm_free(&newcom);
	}
	*mmem = nowmem;
	*ibase =  rbase;
...
#ifdef DEBUG
		MPI_Barrier(Comm);
		DEBUGPRINT("P%d is now exiting from mpirks\n", myid);
#endif
	return;
} 


/* This is to migrate particles on the hierarchical network structure */
void old_pmigrate(void **ibase,ptrdiff_t *mmem, DoDeInfo *ddinfo, GridInfo *gridinfo){
	MPI_Status status;
	MPI_Comm com;
	int src,dest;
	ptrdiff_t nrecv,nsend;
    ...
    nowmem += nmem;
	my_free(rbase);
	if(subgroupsize > 1) old_pmigrate(&base, &nowmem, ddinfo+1,gridinfo);
	*mmem = nowmem;
	*ibase =  base;
	return;
}
...
void pmigrate(void **ibase,ptrdiff_t *mmem, DoDeInfo *ddinfo, GridInfo *gridinfo){
	...
	{
		int i;
		treevorork4particletype *left = (treevorork4particletype*) base;
		for(i=0;i<*mmem;i++){
			if(left[i].x < 3.e-20) 
				DEBUGPRINT("P%d/%d has p%d  with x/y = %g %g\n", myid,nid,i,
					left[i].x, left[i].y);
		}
	...
	*/
	...
	for(igroup=1;igroup<nsubgroup;igroup++){
		...
		if(nmem >0) {
			...
			if(targetsubgid==0){
				memmove(tlocalmax, (char*)pivot+targetsubgid*n_size,n_size);
				iflag = (char*)my_malloc(sizeof(char)*nmem);
				ptrdiff_t i;
#ifdef _OPENMP
#pragma omp parallel for schedule(dynamic)
#endif
				for(i=0;i<nmem;i++){
					char *left = bb + n_size*i;
					if(compare(gridinfo,left,tlocalmax)<0) iflag[i] = 1;
					else iflag[i] = 0;
				}
			}
			else if(targetsubgid == nsubgroup-1){ 
...
#pragma omp parallel for schedule(dynamic)
				...
				for(i=0;i<nmem;i++){ 
					char *left = bb + n_size*i;
					if(compare(gridinfo, left, tlocalmin) >= 0) iflag[i] = 1;
					else iflag[i] = 0;
				}
			}
			else {
...
#pragma omp parallel for schedule(dynamic)
				...
				for(i=0;i<nmem;i++){ 
					char *left = bb + n_size*i;
					if(compare(gridinfo, left,tlocalmin)>=0 && 
							compare(gridinfo, left,tlocalmax)<0) iflag[i]= 1;
					else iflag[i] =0;
				}
			}

			nsend = 0;
			for(i=0;i<nmem;i++) if(iflag[i]) nsend++;
			/*
			DEBUGPRINT("P%d/%d is sending %ld to dest P%d/%d : nsize= %ld %ld\n", myid, nid, nsend,dest,nid,
					n_size, sizeof(treevorork4particletype));
			for(i=0;i<nmem;i++) if(iflag[i]) {
				if(iflag[i]) {
					treevorork4particletype *left = (treevorork4particletype*)(bb + i*n_size);
					DEBUGPRINT("P%d/%d p%ld xy= %g %g\n", myid,nid,i, left->x, left->y);
				}

			}
			*/

			char *bak = (char*)my_malloc(sizeof(char)*n_size*nsend);
			ptrdiff_t j=0,k=0;
			for(i=0;i<nmem;i++){
				if(iflag[i]){
					memmove(bak+n_size*j, bb+n_size*i,n_size);
					j++;
				}
				else {
					memmove(bb+n_size*k,bb+n_size*i, n_size);
					k++;
				}
			}
			right = bb+n_size*k;
			memmove( bb+n_size*k, bak, n_size*j);
			my_free(bak);
			my_free(iflag);
		}
		else{
		...
		/*
#ifdef DEBUG
		if(myid==0) DEBUGPRINT("+: %d : (%ld) <-> (%ld) %ld : xyzchip= %c\n",
				igroup,nrecv, nsend,nowmem, xyzchip); fflush(stdout);
#endif
*/
	}
    base = (void *)realloc(base, sizeof(char)*(nowmem+nmem)*n_size);
...
void DeletePtlOutOfLocalBox(void **padbase, ptrdiff_t *npad, ptrdiff_t n_size, PosType width, BoxMinMax *lbox, 
		SimBoxRange *simbox,
		int (*insidebox)(GridInfo *, const void *, BoxMinMax *, PosType *, SimBoxRange *,int , int, enum PadPtl),
			GridInfo *gridinfo, int icenter){
	ptrdiff_t i;
	char *left, *right;
	left = right = (char*)(*padbase);

	for(i=0;i<*npad;i++){
		if(insidebox(gridinfo, right, lbox, &width, simbox, 0, icenter, PadPtl)) {
...
void *ExtractPtls2Send(void *base, ptrdiff_t nmem, void *padbase, ptrdiff_t npad, DoDeInfo *ddinfo, 
	...
	void *sendbuff = (void *)my_malloc(maxsize*n_size);
	run = (char *)padbase;
	isend = 0;
	for(i=0;i<npad_in_current_level;i++){
		if(insidebox(gridinfo, run, &targetbox, &width,&simbox,0, 13, PadPtl)) {
	...
	for(i=0;i<nmem;i++){
		...
		for(pflag=pmin;pflag<=pmax;pflag++) {
			if(insidebox(gridinfo, buff, &targetbox, &width,&simbox,1, pflag, SimPtl)) 
			{
				if(isend >=maxsize) {
...
void justppadding(void *base, ptrdiff_t nmem, void **padbase, ptrdiff_t *npad, 
	...
	ptrdiff_t n_size = ddinfo->n_size;
	int igroup, subgroupsize, subgroupid;
	BoxMinMax mylocalgroupbox,yourlocalgroupbox;
	mylocalgroupbox= ddinfo->lgroup.xyz;
	myid = ddinfo->myid;
	nid = ddinfo->nid;
	com = ddinfo->com;
	int nsubgroup = ddinfo->nsubgroup;
	...
	for(igroup = 1; igroup < nsubgroup; igroup ++){
		...
		src  = (myid - subgroupsize * igroup + nid)%nid;
		int targetsubgid = (subgroupid + igroup+nsubgroup)%nsubgroup;
		MPI_Sendrecv(&mylocalgroupbox, sizeof(BoxMinMax), MPI_BYTE, src,0,
				&yourlocalgroupbox, sizeof(BoxMinMax), MPI_BYTE, dest, 0,
				com, &status);
		ptrdiff_t sizeinbyte2send, sizeinbyte2recv;
#ifdef DEBUG
		DEBUGPRINT("P%d has mylocal %g %g : %g %g : %g %g\n", myid, mylocalgroupbox.xmin, mylocalgroupbox.xmax,
				mylocalgroupbox.ymin, mylocalgroupbox.ymax, mylocalgroupbox.zmin, mylocalgroupbox.zmax);
		DEBUGPRINT("P%d has targetlocalbox %g %g : %g %g : %g %g ::: P%d\n", myid, yourlocalgroupbox.xmin, yourlocalgroupbox.xmax,
				yourlocalgroupbox.ymin, yourlocalgroupbox.ymax, yourlocalgroupbox.zmin, yourlocalgroupbox.zmax, dest);
#endif
		void *buff2send = ExtractPtls2Send(base, nmem, *padbase,*npad, ddinfo,yourlocalgroupbox, 
	...
	DeletePtlOutOfLocalBox(padbase, npad, n_size, width, &mylocalgroupbox, &box, ddinfo->insidebox , gridinfo);
...
void copyselfperiodic(void *base, ptrdiff_t nmem, void **padbase, ptrdiff_t *npad, DoDeInfo *ddinfo, SimBoxRange simbox,PosType width,
	...
	int (*insidebox)(GridInfo *, const void *, BoxMinMax *, PosType *,  SimBoxRange *, int , int,
			enum PadPtl) = ddinfo->insidebox;
	int (*edgeptl)(GridInfo *, const void *, SimBoxRange *, PosType *) = ddinfo->edgeptl;
	ptrdiff_t i,maxsize;
	char *run;
	BoxMinMax mylocalgroupbox = ddinfo->lgroup.xyz;
#ifdef DEBUG
	DEBUGPRINT("P%d has mylocalgroupbox %g %g : %g %g : %g %g\n", ddinfo->myid, mylocalgroupbox.xmin, mylocalgroupbox.xmax,
			mylocalgroupbox.ymin, mylocalgroupbox.ymax, mylocalgroupbox.zmin, mylocalgroupbox.zmax);
#endif
	...
	for(i=0;i<nmem;i++){
		if(edgeptl(gridinfo, run,&simbox,&width)) {
			...
			for(pflag=0;pflag<27;pflag++) {
				...
				else if(insidebox(gridinfo, buff, &mylocalgroupbox, &width,&simbox,1, pflag, SimPtl)){
					if(*npad >=maxsize) {
						maxsize += stepsize;
						*padbase = (void*)realloc(*padbase, maxsize*n_size);
					}
...
void *SimpleExtractPtls2Send(void *padbase, ptrdiff_t npad, DoDeInfo *ddinfo, 
	...
	ptrdiff_t maxsize = MAX(npad_in_current_level/4,10000);
	ptrdiff_t stepbuffersize = maxsize*0.2;
	void *sendbuff = (void *)my_malloc(maxsize*n_size);
	run = (char *)padbase;
	isend = 0;
	...
	for(i=0;i<npad_in_current_level;i++){
		if(insidebox(gridinfo, run, &targetbox, &width,&simbox,0, icenter, PadPtl)) {
			memcpy((char*)sendbuff+isend*n_size, run, n_size);
			isend ++;
			if(isend >= maxsize) {
				maxsize += stepbuffersize;
				sendbuff = (void *)realloc(sendbuff,maxsize*n_size);
			}
		}
		run += n_size;
...
void adv_justppadding(void **padbase, ptrdiff_t *npad, 
		DoDeInfo *ddinfo, SimBoxRange box, PosType width, GridInfo *gridinfo, int icenter){
	MPI_Status status;
	MPI_Comm com;
	int src, dest;
	ptrdiff_t nrec, nsend;
	...
	for(igroup = 1; igroup < nsubgroup; igroup ++){
		...
		MPI_Sendrecv(&mylocalgroupbox, sizeof(BoxMinMax), MPI_BYTE, src,0,
				...
				com, &status);
		ptrdiff_t sizeinbyte2send, sizeinbyte2recv;
		void *buff2send = SimpleExtractPtls2Send(*padbase,*npad, ddinfo,yourlocalgroupbox, 
				width, box, &sizeinbyte2send, npad_in_current_level, gridinfo, icenter);
		MPI_Sendrecv(&sizeinbyte2send,1,MPI_INT64_T,dest,0, 
				&sizeinbyte2recv,1,MPI_INT64_T,src,0, 
				com,&status);
		*padbase = (void *) realloc( (*padbase),(*npad*n_size + sizeinbyte2recv));
		...
		my_MPI_Sendrecv(buff2send,sizeinbyte2send,MPI_BYTE, dest, 0, 
				...
				com, &status);
		*npad += sizeinbyte2recv/n_size;
		if(buff2send) my_free(buff2send);
	}
	DeletePtlOutOfLocalBox(padbase, npad, n_size, width, &mylocalgroupbox, &box, ddinfo->insidebox , gridinfo, icenter);
	if(subgroupsize >1) adv_justppadding(padbase, npad, ddinfo+1, box,width, gridinfo, icenter);
}
void DeleteInsideLocalBox(void **padbase, ptrdiff_t *npad, ptrdiff_t n_size, BoxMinMax *lbox,SimBoxRange *simbox,
		int (*insidebox)(GridInfo *, const void *, BoxMinMax *, PosType *, SimBoxRange *,int , int, enum PadPtl), 
		GridInfo *gridinfo, int icenter){
	ptrdiff_t i;
	char *left, *right;
	...
	left = right = (char*)(*padbase);
	for(i=0;i<*npad;i++){
		if(!insidebox(gridinfo, right, lbox, &ww, simbox, 0, icenter, PadPtl)) {
			memmove(left,right,n_size);
			left += n_size;
		}
		right += n_size;
...
void ppadding(void *base,ptrdiff_t nmem, void **padbase, ptrdiff_t *npad,
	...
	int (*insidebox)(GridInfo *, const void *, BoxMinMax *, PosType *,  SimBoxRange *, int , int, enum PadPtl) 
		= ddinfo->insidebox;
	void (*shift2pos)(GridInfo *, const void *) = ddinfo->shift2pos;
	ptrdiff_t i,j,maxsize;
	maxsize = 100+ nmem * 0.1;
	ptrdiff_t stepsize = maxsize * 0.1;
	*padbase = (void*)my_malloc(maxsize*n_size);
	PosType ww = 0;
	int nneigh;
	if(ndim ==1) nneigh = 3;
	else if (ndim ==2) nneigh = 9;
	else if (ndim == 3) nneigh = 27;
	else if (ndim == 4) nneigh = 81;
	int icenter = (nneigh+1)/2 -1;

#ifdef _OPENMP
	int *iflag = (int*)my_malloc(nmem*sizeof(int));

	int BitFlag[32];
	for(i=0;i<32;i++) BitFlag[i] = (1<<i);



	/*
	DEBUGPRINT("P has %d mybox: %g %g %g :: %g %g %g\n", n_size, 
			mybox.xmin, mybox.ymin, mybox.zmin,
			mybox.xmax,mybox.ymax,mybox.zmax);
	DEBUGPRINT("P has %d simbox: %g %g %g :: %g %g %g\n",  n_size, 
			simbox.x.min, simbox.y.min, simbox.z.min,
			simbox.x.max,simbox.y.max,simbox.z.max);
	DEBUGPRINT("It has nmem= %ld\n", nmem);
	*/

#define SET_PADDING(a,ii) (a |= BitFlag[ii])
#define UNSET_PADDING(a,ii) (a &= (~BitFlag[ii]))
#define IS_PADDING(a,ii) (a & BitFlag[ii])

	ptrdiff_t npadd = 0;
#pragma omp parallel for schedule(dynamic) reduction(+: npadd)
	for(i=0;i<nmem;i++){
		char *run = (char*)base + i*n_size;
		iflag[i] = 0x00;

		if(insidebox(gridinfo, run, &mybox, &ww, &simbox, 0, icenter, SimPtl) && 
				!insidebox(gridinfo, run, &mycorebox, &ww, &simbox, 0, icenter, SimPtl)){
			SET_PADDING(iflag[i],icenter);
			npadd ++;
			char buff[n_size];
			int pflag;
			for(pflag =0; pflag < nneigh; pflag ++){
				if(pflag == icenter) continue;
				if(insidebox(gridinfo, run, &Bbox, &ww, &simbox, 0, pflag, SimPtl)) { 
					SET_PADDING(iflag[i],pflag);
					npadd ++;
				}
			}
		}
	}
	*padbase = (void*)realloc(*padbase, npadd * n_size);
	npadd = 0;
	char *padrun = (char*)(*padbase);
	for(i=0;i<nmem;i++){
		char *run = (char*)base + i*n_size;
		if(IS_PADDING(iflag[i],icenter)){
			int pflag;
			for(pflag=0;pflag<nneigh;pflag++){
				if(IS_PADDING(iflag[i],pflag)){
					if(pflag==icenter){
						memcpy( padrun, run, n_size);
#ifdef XYZDBL
						shift2pos(gridinfo,padrun);
#endif
					}
					else {
						char buff[n_size];
						memcpy(buff, run, n_size);
						insidebox(gridinfo, buff, &Bbox, &ww, &simbox, 1, pflag, SimPtl);
						memcpy( padrun, buff, n_size);
					} 
					padrun += n_size;
					npadd ++;
				}
...
#undef IS_PADDING
	...
	for(i=0;i<nmem;i++){
		if(insidebox(gridinfo, run, &mybox, &ww, &simbox, 0, icenter, SimPtl) && 
...
#ifdef XYZDBL
			shift2pos(gridinfo,(char*)(*padbase)+(*npad)*n_size);
#endif
			...
			for(pflag =0; pflag < nneigh; pflag ++){
				if(pflag == icenter) continue;
				memcpy(buff, run, n_size);
				if(insidebox(gridinfo, buff, &Bbox, &ww, &simbox, 1, pflag, SimPtl)) { 
	...
	*padbase = (void *)realloc(*padbase, *npad*n_size);
#endif
	adv_justppadding(padbase, npad,ddinfo, simbox, width, gridinfo, icenter);
	DeleteInsideLocalBox(padbase, npad, n_size, &mybox, &simbox, ddinfo->insidebox, gridinfo, icenter);
}
