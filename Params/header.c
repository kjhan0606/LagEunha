	...
	while(strcmp(P_Closing,fgets(line, MAX_LINE_LENGTH,fp)) != 0){
		...
		if(line[0] != '#'){
			if(strstr(line,"define") != NULL) {
				FILE_HEADER(sscanf,line,&,simpar);
				/* backward compat: old AV parameter names */
				ncnt += sscanf(line,P_VORO_AlphaVis_OLD,&GAS_AlphaVis(simpar));
				ncnt += sscanf(line,P_VORO_BetaVis_OLD,&GAS_BetaVis(simpar));
...
void mk_default_kh_param(SimParameters *defsim){
		...
		HUBBLE(defsim) = 1;
		GAS_EVOLMETHOD(defsim) = 1;    // RK4
		GAS_FCENTROID(defsim) = 0.1;
		GAS_GAMMA(defsim) = 5.0/3.0;

		/* KH IC: Lecoanet+2016 setup */
		KH_Rho1(defsim) = 1;
		KH_Rho2(defsim) = 2;
		KH_Vel1(defsim) = 0.5;      // outer velocity
		KH_Vel2(defsim) = -0.5;     // inner velocity
		KH_Deltay(defsim) = 0.025;  // tanh width 'a'
		KH_Pressure(defsim) = 2.5;
		KH_Vperturb(defsim) = 0.01;
		KH_XMIN(defsim) = 0;
		KH_XMAX(defsim) = 1;
		KH_YMIN(defsim) = 0;
		KH_YMAX(defsim) = 1;       // square domain [0,1]x[0,1]
		...
		/* Dissipation: Monaghan AV (av_mode=0) */
		GAS_AVMODE(defsim) = 0;
...
void mk_default_rt_param(SimParameters *defsim, int iflag){
	...
	GAS_REYNOLDS(defsim) = INFINITY;
		...
		NY(defsim) = 256;
		NZ(defsim) = 1;
		HydroGridSize(defsim) = (RT_XMAX(defsim)-RT_XMIN(defsim))/NY(defsim)*4;
		GAS_ETAVIS(defsim) = HydroGridSize(defsim)/4.;
		DetermineFreeFallTime(defsim);
		RT_DEN1(defsim) = 1;
		RT_DEN2(defsim) =  2;
		GAS_ACCY(defsim) = -0.5L;
		GRAVITY(defsim) = 'N';
...
void mk_default_kepler_param(SimParameters *defsim){
	...
	NY(defsim) = 256; 
	NZ(defsim) = 1; 
	HydroGridSize(defsim) = (KP_XMAX(defsim)-KP_XMIN(defsim))/NX(defsim)*4; 
	DetermineFreeFallTime(defsim); 
	GRAVITY(defsim) = 'N'; 
	KP_OA(defsim) = 0.33333;
	KP_EPS(defsim) = 0.1;
    KP_Kappa(defsim) = 0.4;
...
void mk_default_2dglass_param(SimParameters *defsim, int flag){
    ...
    NY(defsim) = 256;
    NZ(defsim) = 1;
    HydroGridSize(defsim) = (GL2D_XMAX(defsim)-GL2D_XMIN(defsim))/NX(defsim)*4;
    DetermineFreeFallTime(defsim);
    GRAVITY(defsim) = 'N';
    GL2D_OA(defsim) = 0.333333;
    GL2D_EPS(defsim) = 0.1;
    GL2D_Kappa(defsim) = 0.4;
...
//	RT_ACC(defsim) = 0;
	...
	if(strcmp(cosmology,"WMAP3")==0 || strcmp(cosmology,"BWMAP3")==0){
		...
		mk_default_rt_param(defsim, 0);
	}
	else if(strcmp(cosmology,"RT_LF")==0){
		mk_default_rt_param(defsim, 1);
	}
